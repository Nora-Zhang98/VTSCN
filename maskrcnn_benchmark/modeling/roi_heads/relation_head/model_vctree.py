...
class VCTreeLSTMContext(nn.Module):
    ...
    def forward(self, x, proposals, rel_pair_idxs, global_features, union_features, logger=None, all_average=False, ctx_average=False):
        ...
        return obj_dists, obj_preds, edge_ctx, bi_preds, tri_rep, union_features
    ...
    def vctree_score_net(self, num_objs, roi_feat, roi_dist, proposals):
        ...
        for sub, obj, dist, prp in zip(sub_feats, obj_feats, roi_dists, proposals):
            ...
            joint_pred =  torch.sigmoid(vis_prior) *  co_prior # sigmoid(8)*1369 sigmoid(4)

            bi_preds.append(joint_pred) # 0.27左右 18
