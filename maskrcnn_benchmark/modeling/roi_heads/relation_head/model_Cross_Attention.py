...
class Single_Layer_Cross_Attention(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.CA_Cell_vis = Cross_Attention_Cell(config, hidden_dim=4096)
        self.CA_Cell_txt = Cross_Attention_Cell(config, hidden_dim=4096)
...
class CA_Encoder(nn.Module):
    ...
    def __init__(self, config, n_layers):
        ...
        self.num_head = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.NUM_HEAD # 2
        self.inner_dim = self.cfg.MODEL.ROI_RELATION_HEAD.TRANSFORMER.INNER_DIM # 512
...
class CA_Context(nn.Module):
    ...
    def forward(self, roi_features, proposals, global_features, rel_pair_idxs, union_features, logger=None):
        ...
        obj_pre_rep_txt = self.lin_obj_textual(obj_pre_rep_txt)# 200→512
