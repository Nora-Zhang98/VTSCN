...
def get_current_predicate_idx(incremental_stage_list, zeros_vector_penalty, Dataset_choice):
    ...
    for i in range(len(incremental_stage_list)):
        for num in incremental_stage_list[i]:
            outp[num] = i+1 # 给50类谓语做mask，1111,22222...
    max_p = []
    ...
    for i in range(len(incremental_stage_list)):
        ...
        for j in range(data_long):
            p1.append(0)
            p2.append(zeros_vector_penalty) # 每个初始值都是0.1
        max_l = max_p[i]
...
def generate_sample_rate_vector(Dataset_choice, num_stage_predicate):
    ...
    outp = []
    for i in range(len(num_stage_predicate)): # [4,10,19,38,50]
        opiece = []
        for j in range(len(predicate_new_order_count)): # len51
            opiece.append(0.0) # 51个0
        num_list = predicate_new_order_count[0:(num_stage_predicate[i]+1)]
        median = np.median(num_list[1:]) # 第一个是bg，中位数不考虑它
        for j in range(len(num_list)):
            if num_list[j] > median:
                num = median / num_list[j] # 中位数确定重采样率
                if j == 0:
                    num = num * 10.0
                if num < 0.01:
                    num = 0.01
